{ pkgs, pkgsLib, wrappedPkgs, shellStartCmd }:
pkgs.writeShellApplication {
  name = "setupWorkspaces";
  # `shellStartCmd` is passed by swayDecl.nix: the shellCacher launcher's
  ...
  text = ''
    ...
    swaymsg exec ${pkgsLib.getExe pkgs.kitty} ${shellStartCmd}
    ...
    swaymsg "workspace 2; exec ${pkgsLib.getExe wrappedPkgs.kitty} ${shellStartCmd}"
  '';
}
